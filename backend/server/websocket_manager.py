...
def _augment_investor_query_domains(task: str, query_domains: List[str]) -> List[str]:
    if not task:
        return query_domains

    normalized_task = task.lower()
    is_investor_task = "investor" in normalized_task and "portfolio" in normalized_task
    if not is_investor_task:
        return query_domains
    ...
    existing = set(query_domains or [])
    return list(existing.union(preferred_domains))
...
async def run_agent(task, report_type, report_source, source_urls, document_urls, tone: Tone, websocket, stream_output=stream_output, headers=None, query_domains=[], config_path="", return_researcher=False, mcp_enabled=False, mcp_strategy="fast", mcp_configs=[], max_search_results=None):
    ...
    query_domains = _augment_investor_query_domains(task, query_domains)
