...
import { buildInvestorQuery } from '../utils/investorTask';
...
const APOLLO_INVESTOR_TYPES = new Set(["VC", "Private Equity"]);
...
export const GPTResearcher = ({
  ...
  const [chatBoxSettings, setChatBoxSettings] = useState<ChatBoxSettings>({ 
    ...
    mcp_configs: [],
    mcp_strategy: 'fast',
    company_search_provider: 'apollo',
  });
  ...
  const companySearchProvider = chatBoxSettings.company_search_provider ?? 'apollo';
  const setCompanySearchProvider = (value: string) => {
    setChatBoxSettings((prevSettings) => ({
      ...prevSettings,
      company_search_provider: value,
    }));
  };

  useEffect(() => {
    if (companySearchProvider !== 'apollo') {
      return;
    }
    if (!investorType || APOLLO_INVESTOR_TYPES.has(investorType)) {
      return;
    }
    setInvestorType("VC");
  }, [companySearchProvider, investorType]);
  ...
  const handleDisplayResult = async (newQuestion: string) => {
    ...
    const taskToRun = buildInvestorQuery(newQuestion, {
      investorType,
      hqCountry,
      industry,
      companyCount,
    ...
    setShowResult(true);
    setLoading(true);
    setQuestion(taskToRun);
    setPromptValue("");
    setAnswer("");
    setOrderedData((prevOrder) => [...prevOrder, { type: 'question', content: taskToRun }]);
    ...
    initializeWebSocket(taskToRun, chatBoxSettings, {
      investorType,
      hqCountry,
      industry,
      companyCount,
