...
export const useWebSocket = (
  ...
  const initializeWebSocket = useCallback((
    ...
    investorFilters?: InvestorFilterPayload
  ) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      newSocket.onopen = () => {
        ...
        const { report_type, report_source, tone, mcp_enabled, mcp_configs, mcp_strategy, company_search_provider } = chatBoxSettings;
        const companySearchProvider = company_search_provider || "apollo";
        const keywordTags = [
          investorFilters?.industry?.trim(),
          investorFilters?.investorType?.trim(),
        ].filter((tag): tag is string => Boolean(tag));
        const organizationLocations = investorFilters?.hqCountry?.trim()
          ? [investorFilters.hqCountry.trim()]
          : undefined;
        ...
        try {
          ...
          const dataToSend = { 
            ...
            headers: {
              retriever: companySearchProvider,
              investor_type: investorFilters?.investorType,
              organization_locations: organizationLocations,
              organization_keyword_tags: keywordTags.length ? keywordTags : undefined,
              company_count: investorFilters?.companyCount,
            },
          };
