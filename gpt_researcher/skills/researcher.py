...
class ResearchConductor:
    ...
    async def plan_research(self, query, query_domains=None):
        ...
        Returns:
            List of queries
        """
        retriever_names = [r.__name__.lower() for r in self.researcher.retrievers]
        non_mcp_retrievers = [name for name in retriever_names if "mcpretriever" not in name]
        apollo_only = bool(non_mcp_retrievers) and all("apolloorganizationsearch" in name for name in non_mcp_retrievers)

        if apollo_only:
            await stream_output(
                "logs",
                "planning_research",
                f"🏢 Using Apollo company discovery for: {query}...",
        ...
        search_debug = getattr(self.researcher, "last_search_debug", None)
        if search_debug:
            await stream_output(
                "search_debug",
                "search_debug",
    ...
    async def _process_sub_query(self, sub_query: str, scraped_data: list = [], query_domains: list = []):
        ...
        try:
            ...
            web_context = ""
            non_mcp_retrievers = [r for r in self.researcher.retrievers if "mcpretriever" not in r.__name__.lower()]
            apollo_only = bool(non_mcp_retrievers) and all(
                "apolloorganizationsearch" in r.__name__.lower() for r in non_mcp_retrievers
            )
            ...
            if not scraped_data:
                if apollo_only:
                    scraped_data = await self._search_data_without_scraping(sub_query, query_domains)
                else:
            ...
            if combined_context:
                ...
                if self.researcher.verbose:
                    ...
                    context_label = "web content" if web_available else "no web content"
                    if apollo_only:
                        context_label = "Apollo company data" if web_available else "no Apollo company data"
                    await stream_output(
                        "logs",
                        "context_combined",
                        f"📚 Combined research context: {mcp_count} MCP sources{cache_status}, {context_label}",
                        self.researcher.websocket,
                    )
            else:
    ...
    async def _search_data_without_scraping(self, sub_query: str, query_domains: list | None = None):
        if query_domains is None:
            query_domains = []
            ...
            try:
                ...
                for result in search_results:
                    if not isinstance(result, dict):
                        continue
                    raw_content = str(result.get("body") or "").strip()
                    href = str(result.get("href") or "").strip()
                    if not raw_content and not href:
                        continue
                    pages.append(
                        {
                            "raw_content": raw_content or href,
                            "url": href,
                            "title": str(result.get("title") or href or "Apollo result"),
                        }
                    )
            except Exception as e:
                self.logger.error(f"Error searching without scraping with {retriever_class.__name__}: {e}")

        if self.researcher.verbose:
            await stream_output(
                "logs",
                "apollo_company_discovery",
                f"🏢 Apollo company discovery returned {len(pages)} candidate records without web scraping.",
                self.researcher.websocket,
            )
