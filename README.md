...
## � Discord Community Search (Hiring Signals)
The lead discovery dashboard can scan configured Discord channels for hiring intent. The backend uses `chat-exporter` to generate transcripts and the existing OpenAI pipeline to extract structured hiring signals.

### Required Discord Bot Permissions
- View Channels
- Read Message History
- Read Messages (Message Content intent must be enabled in the Discord developer portal)

### Configuration
Set the Discord settings in `.env` or `CONFIG_PATH` JSON:
- `DISCORD_BOT_TOKEN`
- `DISCORD_CHANNEL_IDS` (array of channel IDs)
- `DISCORD_LOOKBACK_HOURS`, `DISCORD_MESSAGE_LIMIT`
- `DISCORD_TRANSCRIPT_DIR` for transcript artifacts
- `DISCORD_PREFILTER_ENABLED` and `DISCORD_PREFILTER_KEYWORDS`

### How It Works
1. The community search button calls `/api/community/search` when Discord is selected as the source.
2. The service fetches recent messages, exports transcripts with `chat-exporter`, and groups message windows for context.
3. The OpenAI extraction pipeline returns structured hiring signals with message evidence and source metadata.
